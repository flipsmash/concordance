...
@pg
def test_fill_definitions_casts_out_a_foreign_or_archaic_spelling_hit(monkeypatch):
    # A source can successfully define a word that still shouldn't be
    # accepted: a foreign word (acte -- French) or an archaic/OCR spelling of
    # a common modern word (assunder -- asunder) -- neither is caught by
    # junk_pos_reason (both come back as ordinary nouns/adverbs), so
    # fill_definitions needs its own variant_reject_reason check to cast them
    # out rather than accept them just because SOME source produced a gloss.
    from concordance import resolve
    from concordance.model import Candidate, RejectReason
    ...
    def fake_freedict(cand, session):
        ...
        elif cand.lemma == "assunder":
            cand.definition = "into separate parts or pieces"
            cand.definition_source = "Free Dictionary API"
            cand.part_of_speech = "adverb"
    ...
    schema = "cc_test_variant_castout"
    conn = db.connect(_URL)
    with conn.cursor() as cur:
        cur.execute(f"DROP SCHEMA IF EXISTS {schema} CASCADE")
    conn.commit()
    ...
    words = [Candidate(lemma=l, pos="NOUN") for l in ("acte", "assunder", "armiger")]
    db.sync_book_results(conn, "Book One", kept=words, rejected=[], schema=schema)

    stats = db.fill_definitions(conn, schema)

    assert stats["cast_out"] == 2   # acte (foreign) + assunder (archaic spelling)
    assert stats["defined"] == 1    # armiger -- a real word -- survives

    with conn.cursor() as cur:
        cur.execute(f"select active from {schema}.word where lemma='acte'")
        assert cur.fetchone() == (False,)
        cur.execute(f"select active from {schema}.word where lemma='assunder'")
        assert cur.fetchone() == (False,)
        cur.execute(f"select active, definition from {schema}.word where lemma='armiger'")
        assert cur.fetchone() == (True, "a person entitled to bear heraldic arms")
